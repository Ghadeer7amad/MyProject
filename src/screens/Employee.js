    ...
    <View style={{marginTop: 30}}>
      ...
      <ScrollView snapToInterval={screenwidth} decelerationRate='fast' alwaysBounceHorizontal={true} horizontal>
        {items.map((item, index) => (
          ...
          marginHorizontal: 5}}>
            <Image source={{uri: item?.image?.secure_url}} style={{width: screenwidth-180, height: screenheight - 600, resizeMode: 'contain', borderTopLeftRadius: 60, borderTopRightRadius: 60 }} />
             <Text style={{color:'#fff', letterSpacing: 1.5, marginBottom: 3}}>{item.name}</Text>
            <Text style={{color:  Color.secondary, marginBottom: 3}}>{item.experienceYears} years of experience</Text>
